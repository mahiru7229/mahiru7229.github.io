...
const PAGE_SIZE = 100;
const MAX_PAGES = 10;
const CACHE_KEY = "mcw-launcher-release-cache-v3";
...
function renderChannelState() {
  ...
  const releaseBrowser = $("release-browser") || document.querySelector(".release-browser");
  if (releaseBrowser) releaseBrowser.classList.toggle("beta-mode", isBeta);
...
function initialize() {
  ...
  renderEverything();
  setDataStatus(
    cache?.releases?.length
      ? "Đang hiển thị dữ liệu đã lưu · đang kiểm tra cập nhật"
      : "Đang hiển thị dữ liệu dự phòng · đang kết nối GitHub",
    "ready"
  );
  window.setTimeout(() => loadReleases(), 0);
}
